...
%figure
%imshow(img)
topLeft = [round(sortedFiducial(1,1)-3*bitSize), round(sortedFiducial(1,2)-3*bitSize)];
qrSize = round(bitSize*41);
bottomRight = [round(topLeft(1)+qrSize), round(topLeft(2)+qrSize)];
